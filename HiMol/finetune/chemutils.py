...
def tree_decomp(mol):
    ...
    for i in range(len(cliques)):
        if len(cliques[i]) <= 2: continue   #选择cliques中 >2个原子的list
        for atom in cliques[i]:    #遍历该list中原子
            for j in nei_list[atom]:  #遍历该原子所在的所有cliques中的list
                if i >= j or len(cliques[j]) <= 2: continue  #如果cliques存在第i个list之前有第j个list也是原子数>2
                inter = set(cliques[i]) & set(cliques[j])
                if len(inter) > 2:
                    cliques[i].extend(cliques[j]) 
                    cliques[i] = list(set(cliques[i])) #将第i和j内的原子去重都放在第i个（靠后）list中
                    cliques[j] = [] #第j个list清空

    cliques = [c for c in cliques if len(c) > 0] #清除合并后为空的list
    nei_list = [[] for i in range(n_atoms)]  #重新更新
    for i in range(len(cliques)):
        for atom in cliques[i]:
            nei_list[atom].append(i)
